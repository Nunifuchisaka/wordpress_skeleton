...
記事を追加・変更してリポジトリに残したいときは `npm run db:export` してからコミットしてください（画像を追加した場合は `.gitignore` のuploads除外も確認）。
...
- **hoge** — 最小構成のサンプルプラグイン
- **Demo Styles**（`demo-styles`）— デモテンプレートに最低限のスタイルを当てる開発用プラグイン。テーマ本体のCSSを汚染しないための分離で、実案件では無効化またはディレクトリごと削除する
